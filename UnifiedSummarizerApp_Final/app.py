...
class UnifiedSummarizerApp:
    def __init__(self):
        # Safely initialize embedding service
        try:
            self.embedding_service = EmbeddingService()
            logger.info("✅ EmbeddingService loaded successfully.")
            # Optional: self.test_embedding_model()
        except Exception as e:
            logger.error(f"❌ Failed to initialize EmbeddingService: {e}")
            st.error("⚠️ Embedding model couldn't be loaded. Please check your environment or model configuration.")
    ...
    def run(self):
        try:
            ...
            logger.error(f"❌ App crashed during run: {e}")
            st.error("🚨 App failed to start. Please check logs for details.")

    # Optional test (can be removed if not needed)
    def test_embedding_model(self):
        try:
            self.embedding_service.get_embedding("Test sentence")
            logger.info("✅ Embedding model responded successfully.")
        except Exception as e:
